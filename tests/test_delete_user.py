...
#                 # Step 3: Delete user data on the server
#                 delete_response = requests.delete(f"{api_url}/user/delete/{user_id}/", headers=headers)
#                 assert delete_response.status_code == 204, "Failed to delete user data"  # Status code for successful deletion is 204
#                 print("User deleted successfully.")

#                 # Optionally, remove the user from the local JSON data
#                 users_data = [user for user in users_data if user["email"] != user_creation_data["email"]]  # Remove user from JSON data
